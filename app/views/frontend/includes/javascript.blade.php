<!--INIT JAVASCRIPT!-->

{{ HTML::script('bower_components/jquery/dist/jquery.min.js') }}

{{ HTML::script('bower_components/bootstrap/dist/js/bootstrap.min.js') }}

{{ HTML::script('bower_components/metisMenu/dist/metisMenu.min.js') }}

{{ HTML::script('js/frontend/bootstrap.file-input.js') }}

{{ HTML::script('js/frontend/sb-admin-2.js') }}

{{ HTML::script('js/frontend/sb-admin-2.js') }}

{{ HTML::script('js/frontend/sweet-alert.min.js') }}

{{ HTML::script('js/frontend/owl.carousel.js') }}

{{ HTML::script('js/frontend/emoticons.js') }}

{{ HTML::script('js/frontend/underscore.js') }}

{{ HTML::script('js/frontend/generic.js') }}

{{ HTML::script('js/frontend/bootstrap-datepicker.js') }}

{{ HTML::script('js/frontend/bootstrap-datepicker.es.min.js') }}

{{ HTML::script('js/frontend/joint-master/lib/jquery.js') }} 

{{ HTML::script('js/frontend/joint-master/lib/lodash.js') }} 

{{ HTML::script('js/frontend/joint-master/lib/backbone.js') }} 

{{ HTML::script('js/frontend/joint-master/src/core.js') }} 

{{ HTML::script('js/frontend/joint-master/src/vectorizer.js') }} 

{{ HTML::script('js/frontend/joint-master/src/geometry.js') }} 

{{ HTML::script('js/frontend/joint-master/src/joint.dia.graph.js') }} 

{{ HTML::script('js/frontend/joint-master/src/joint.dia.cell.js') }} 

{{ HTML::script('js/frontend/joint-master/src/joint.dia.element.js') }} 

{{ HTML::script('js/frontend/joint-master/src/joint.dia.link.js') }} 

{{ HTML::script('js/frontend/joint-master/src/joint.dia.paper.js') }} 

{{ HTML::script('js/frontend/joint-master/plugins/joint.shapes.basic.js') }} 

{{ HTML::script('js/frontend/joint-master/plugins/routers/joint.routers.orthogonal.js') }} 

{{ HTML::script('js/frontend/joint-master/plugins/routers/joint.routers.metro.js') }} 

{{ HTML::script('js/frontend/joint-master/plugins/connectors/joint.connectors.normal.js') }} 

{{ HTML::script('js/frontend/joint-master/plugins/connectors/joint.connectors.rounded.js') }} 

{{ HTML::script('js/frontend/joint-master/plugins/connectors/joint.connectors.smooth.js') }} 
<!--END JAVASCRIPT!-->